...
internal class Gogoanime(context: AnimeLoaderContext) : ParsedAnimeParser(context) {
    ...
    override val configKeyDomain: ConfigKey.Domain = ConfigKey.Domain("gogoanime3.co")

    override suspend fun getList(offset: Int, order: SortOrder, filter: AnimeListFilter): List<Anime> {
        val page = filter.page
        val query = filter.query
        val url = if (!query.isNullOrBlank()) {
            "https://$domain/search.html?keyword=$query&page=$page"
        } else {
            "https://$domain/popular.html?page=$page"
        }
